...
from cms.emails import send_new_comment_email, send_new_reply_email
from cms.extensions import db
from cms.forms import CommentForm, AdminCommentForm
from cms.models import Post, Category, Comment
...
def index():
    ...
    per_page = current_app.config['CMS_POST_PER_PAGE'] # 每页数量
...
def show_category(category_id):
    ...
    per_page = current_app.config['CMS_POST_PER_PAGE']
...
def show_post(post_id):
    ...
    if current_user.is_authenticated: # 已登录，使用管理员表单
        ...
        form.email.data = current_app.config['CMS_EMAIL']
    ...
    if form.validate_on_submit():
        ...
        if current_user.is_authenticated:
            ...
            # admin_email = current_app.config['CMS_EMAIL']
...
def change_theme(theme_name):
    if theme_name not in current_app.config['CMS_THEMES'].keys():
        abort(404)
